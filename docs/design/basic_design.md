...
CIMは、Lutronシステムのコミッショニング業務において発生するIssueを管理するためのWebアプリケーションである。

利用者はProjectを選択し、Issueの登録・更新・確認を行う。

AI（Ollama）は音声入力を解析し、Issue Draftの作成を支援する。
...
初期版はWindows 11へデプロイする。

アプリケーションはOS依存を避け、将来的にUbuntu Serverへ移行可能な設計とする。
...
| モデル        | 説明                     |
| ---------- | ---------------------- |
| Hotel      | Projectを管理する施設・建物      |
| Project    | コミッショニング対象となる案件        |
| User       | システム利用者                |
| RoomType   | 部屋種別                   |
| Room       | Project内の部屋            |
| Issue      | コミッショニング時に発生した課題・確認事項  |
| Comment    | Issueに対するコメント履歴        |
