...
|設計原則|内容|
|---|---|
|User in Control|最終判断は常にユーザーが行う。|
...
|User|システムユーザー (username、password_hash、display_name、role を持つ)|
|RoomType|部屋種別|
|Room|Hotel 内の部屋|
...
Issue 管理は本システムの中心機能である。

ユーザーは Issue の登録、更新、参照および Status の変更を行う。
...
ユーザーは username と password により認証後、システムを利用する。
...
- ユーザーに理解しやすいエラーメッセージを表示する。
- システム内部の詳細情報はユーザーへ表示しない。
- システム内部エラーはログへ記録する。
- エラー発生時もデータ整合性を維持する。
